...
function showList(data, page) {
    for (var i = 7 * (page - 1); i < 7 * page; i++) {
        console.log(data[i]);
        if (data[i]) {
            var formateDate = moment(data[i].create_time).toString();
            var dateArr = formateDate.split(' ');
            var date = dateArr[1] + ' ' + dateArr[2] + '，' + dateArr[3];
            var fileName = data[i].md.split('.')[0];
            ul.append("<li>" + "<h3>" + date + "</h3>" + "<h2><a>" + fileName + "</a></h2>" + "<blockquote>" + data[i].tag_name + "</blockquote>" + "</li>");
        }
    }
}

},{"../api/index":1,"jquery":29,"moment":30}],3:[function(require,module,exports){
